...
class Sistema extends Config
{
    ...
    public function reset($correo)
    {
        if ($this->checkEmail($correo)) {
            ...
            if (isset($datos[0])) {
                ...
                Para recuperar su contraseña, haga click en el siguiente enlace por favor: <br>
                <a href='http://localhost/ferreteria/admin/login.php?action=RECOVERY&token=" . $token . "'>Recuperar contraseña :3 </a><br>
                <br>
    ...
    public function sendMail($destinatario, $nombre, $asunto, $mensaje)
    {
        ...
        $mail->Password = 'jzaheiihgumnyhom'; 
    ...
    public function registerCita($datos)
    {
        $this->connect();
        try {
            $this->conn->beginTransaction();
            $sql = "INSERT INTO citas (nombre, apellido, email, telefono, fecha_de_cita, hora_de_cita) 
            ...
            $this->alert('danger', 'Error al registrar la cita: ' . $ex->getMessage());
            $this->conn->rollBack();
            return false;
        }
    }
